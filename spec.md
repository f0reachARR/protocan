...
- スキーマハッシュは、生成される **Descriptor Blob のバイナリデータ列** を入力として **コンパイル時に計算** された 32-bit (FNV-1a) ハッシュ値である。入出力構造が単一のバイナリ（詳細は `descriptor_spec.md` を参照）に厳密に定まるため、ジェネレータの環境依存によるハッシュ揺らぎが発生しない。
- これにより、`node_type_id` や `schema_version` といった ID を明示的に割り当てたり手動管理する必要がなくなる。
- ハートビート（NMT）にはデバイス上の全ノードインスタンスの `schema_hash` が含まれ、PC側（ブリッジ）はそのハッシュを見て、未知のノードであればディスクリプタを要求し、既知（キャッシュ済）であれば即座に通信を開始する。
...
Bit [24:18] Source Device ID (7 bit) — 物理デバイス識別
Bit [17:14] Source Local Node (4 bit) — デバイス内論理ノード (0–15)
Bit [13:0]  Sub-field (14 bit) — コンテキスト（Req/Res判定やターゲットノード・識別用シーケンス番号等）依存
```

**【Device ID の割り当てと衝突防止】**
デバイスIDは基板上のDIPスイッチ等で **物理的（静的）に設定** されることを前提とする。同一バス上に同じ Device ID を持つノードが存在するとデータの衝突やバスオフの原因となるため、衝突防止策として各デバイスは「起動直後に一定期間（例: 200ms）Listen-Only モードで待機し、自身と同じ Device ID を持つノードの NMT HEARTBEAT を観測した場合、直ちに ERROR 状態へ遷移して一切の送信を行わない（LED等で異常を通知する）」仕組みを実装することが要求される。
...
各デバイスは周期的にハートビートをブロードキャストする。マスターはこれによりデバイス接続と状態遷移を検知・管理する。

```
■ HEARTBEAT (Device → Master/All)
  ...
  Payload (最大 64 bytes):
    ...
    Per node (num_nodes 回, 各 8 bytes):
      [+0:3] schema_hash  : uint32 LE (プロトコルバージョン等の代わり)
      [+4]   local_node_id: uint8
      [+5]   instance_index: uint8
      [+6:7] (reserved)   : uint16

※ OP (OPERATIONAL) 状態以降はネットワーク帯域節約のため、`num_nodes = 0` としてノード情報を省略したベース部分（6 bytes）のみを送信する。PREOP 状態までは、全ノードの情報を送信しきるまで複数の HEARTBEAT フレームを周期的に送信する（最大7ノード/フレーム）。
```
...
```
■ EMCY_MESSAGE (Device → Master/All)
  CAN ID: EXT_ID(EMCY, device_id, local_node_id, emergency_code)
  Payload:
    [0:1]  error_register : uint16 LE (内部エラーフラグ)
    [2..]  error_data     : packed binary (オプション、詳細エラー情報)
```

### 5.3 DISC (Discovery / 0x2) とブリッジ初期化

マスターがデバイスの持つスキーマ（プロトコル構造や ROS 2 対応情報）を取得するためのプロトコル。
マスターは NMT ハートビートで未知の `schema_hash` を検出すると、以下のディスカバリフローを実行する。

```
■ DISC_GET_DESCRIPTOR (Master → Device)
  CAN ID: EXT_ID(DISC, master, 0, target_device_id << 7 | 0x00)
  Payload:
    [0]    local_node_id : uint8 (0xFF の場合はデバイスが持つ全スキーマを一括要求、それ以外は該当ノードのスキーマのみ要求)
```

※ デバイスからの応答となるディスクリプタバイナリは 64B を超える場合が多いため、自動的に **BULK 転送 (0x5)** を使用してレスポンスされる。マスターはキャッシュ済みの `schema_hash` を持つ既知のノードについては要求をスキップし、未知の `local_node_id` に対するディスクリプタのみを個別に要求することでトラフィックを削減できる。

**初期化シーケンス:**

1. Master: `HEARTBEAT` 受信 (各ノードの Schema Hash を確認)
2. Master: 未知のノードに対して `DISC_GET_DESCRIPTOR` を送信 (キャッシュがない場合)
3. Device: `BULK_TRANSFER` で要求されたノードのディスクリプタ Blob を返答
4. Master: `PDO_CFG` で PDO マッピングを送信し、完了後に `NMT_START` を指示
...
■ PARAM_READ_REQ / PARAM_WRITE_REQ (Requester → Responder)
  CAN ID: EXT_ID(PARAM, requester, 0, target_device_id << 4 | target_local_node)
  Payload:
    [0]    command     = 0x01 (READ) または 0x02 (WRITE)
    [1]    param_index : uint8
    [2..]  value       : packed binary (WRITE の場合のみ、書き込むデータ)

■ PARAM_READ_RES / PARAM_WRITE_RES (Responder → Requester)
  CAN ID: EXT_ID(PARAM, responder, local_node, requester_device_id)
  Payload:
    [0]    command = 0x81 (READ_RES) または 0x82 (WRITE_RES)
...
マスターがデバイスに対して、送受信する Topic データパッキング構造（Standard CAN ID での送受信）を設定する。

```
■ PDO_CFG_BEGIN (Master → Device)
  CAN ID: EXT_ID(PDO_CFG, master_device, 0, target_device_id << 7 | 0x00)
  Payload:
...
■ PDO_CFG_ENTRY (Master → Device, num_entries 回送信)
  CAN ID: EXT_ID(PDO_CFG, master_device, 0, target_device_id << 7 | sequence_num)
  Payload:
    [0]    local_node_id   : uint8
    [1]    topic_index     : uint8
    [2]    field_index     : uint8  (0xFF = トピック全フィールド)
    [3]    offset          : uint8  (PDO ペイロード内のバイトオフセット)
    [4]    size            : uint8  (バイトサイズ)

■ PDO_CFG_COMMIT (Master → Device)
  CAN ID: EXT_ID(PDO_CFG, master_device, 0, target_device_id << 7 | 0x7F)
  Payload:
    [0:1]  pdo_id         : uint16 LE
    [2]    action          : uint8 (0=Apply, 1=Delete)
  
  ※ action=1 (Delete) の場合、事前の `PDO_CFG_ENTRY` 送信は不要である。マスターは `pdo_id` を指定して COMMIT を送るだけで該当するマッピングを解除できる。

■ PDO_CFG_ACK (Device → Master)
  CAN ID: EXT_ID(PDO_CFG, device_id, 0, master_device_id << 7 | 0x00)
  Payload:
    [0:1]  pdo_id         : uint16 LE
...
64B を超えるデータ（ディスクリプタ応答、FWアップデート等）を ISO-TP (ISO 15765-2) 準拠で分割転送する。

```
■ BULK プロトコル
  CAN ID: EXT_ID(BULK, source_device, 0, target_device_id << 7 | transfer_channel)
  
  ※ `transfer_channel` (7 bit) は同時進行する複数のバルク転送を区別するためのコンテキストID。
  
  First Frame:
    [0]   payload_type : uint8 (0=Descriptor, 1=Service Req, 2=Service Res, 3=FW Update, 0xFF=Other)
...
- **データ長 0**: 単なるタイミング同期用のトリガとして使用。
- **データ長 8**: ペイロードを `uint64 LE` 型のグローバルタイムスタンプ（例: マスター起動からの経過マイクロ秒等）として扱い、デバイス側のクロックと同期させる。

※まだ実装しないこと

### 5.8 SERVICE (ROS Service Call / 0x8)

ROS 2 の Service 呼び出し（Request / Response）を実現するための特別なパケット形式。シーケンス ID を用いて1往復のリクエストとレスポンスを紐付ける。

```
■ SERVICE_REQ (Master → Device)
  CAN ID: EXT_ID(SERVICE, master, 0, target_device_id << 4 | target_local_node)
  Payload:
...
■ SERVICE_RES (Device → Master)
  CAN ID: EXT_ID(SERVICE, device, local_node, master << 4)
  Payload:
...
※ リクエスト/レスポンスサイズが 64B を超える場合は、自動的に対応する BULK 転送パケットでラップされる。
...
デバイスの ROM (ファームウェア内) に配置されるディスクリプタは、`protoc` プラグインが生成した **定数バイナリ配列 (Descriptor Blob)** である (`const uint8_t[]`)。
ファームウェア自身はディスクリプタをパースせず、ブリッジからの DISC 要求にこの Blob を応答として転送する機能のみ持つ。

ブリッジ側は受信した Blob をパースし、ROS 2 ノードや Topic、Service の構成を動的に復元する。
> **Note**: Descriptor Blob のバイナリデータレイアウトの詳細仕様については、別紙 `descriptor_spec.md` を参照のこと。このバイナリデータと全く同じ配列が `schema_hash` 計算の入力となる。

### 7.2 C API コードフレームワーク

コード生成される `C API` により、開発者はスキーマの詳細を安全に使える。

```c
// ファームウェアイメージ例 (device_manifest.toml の廃止と C による静的定義)
...
int main(void) {
    ...
    // ノードの初期化と構成追加 (これによってディスクリプタ片が実行時に論理連結される)
