...
│  - constexpr packed 構造体 & コンパイル時エンコーダ/デコーダ       │
│  - ディスクリプタ片 (constexpr ROM) — ROS 2 マッピング情報を含む    │
│  - schema_hash 定数 (constexpr コンパイル時計算)               │
└────────────┬───────────────────────────────────────────────┘
             │  ユーザーが C++ でデバイス構成を記述
             ▼
┌────────────────────────────────────────────────────────────┐
│  デバイスファームウェア (main.cpp)                               │
│  - device.add_node() でノード構成を宣言                         │
...
PDOの最大個数はノードごとにコード生成時に推定し、生成コードに `constexpr` 定数としてハードコードする。
...
`device.poll()` の受信処理ループでマスターからの `PDO_CFG` コマンドがバックグラウンドで処理され、ペリフェラルの Rx マスクフィルタや受信テーブルが自動的に更新される。
...
int main() {
    ...
    // ラムダによる型安全コールバック登録
    motor_left.on_cmd_vel([](const protocan::bldc_motor::TwistCommand& cmd) {
...
コード生成される `C++ API` により、開発者はスキーマの詳細を型安全に使える。複数インスタンスの登録にも対応する。

**C++ 固有の最適化:**

- **`constexpr`**: schema_hash、フィールドオフセット・サイズ、PDO 最大数などをコンパイル時に計算・検証
- **テンプレートベースのゼロコスト抽象化**: 型安全な PDO エンコード/デコード。ランタイムディスパッチなしで型に応じた処理を生成
- **ラムダ / `std::function`**: `void*` を排除した型安全コールバック。クロージャによるコンテキストキャプチャが可能
- **名前空間による Node Type 分離**: `protocan::bldc_motor::Node` のような階層構造で名前衝突を回避

```cpp
// ファームウェアイメージ例

#include <protocan/bldc_motor.hpp>

using namespace protocan;

// Service のコールバック実装
// ラムダで直接登録するか、通常の関数として定義するかは開発者の自由

Status on_set_enable(
    const bldc_motor::SetEnableRequest& req,
    bldc_motor::SetEnableResponse& res) {
...
int main() {
    Device device("DualMotorBoard");

    // ノードの初期化と構成追加 (論理ノードID = 0, 1 として登録)
    bldc_motor::Node motor_left(0, "left");
    device.add_node(motor_left);

    bldc_motor::Node motor_right(1, "right");
    device.add_node(motor_right);

    // Service コールバック登録 (型安全、void* 不要)
    motor_left.on_set_enable(on_set_enable);

    device.start();

    while (true) {
        device.poll();  // 受信/ハートビート処理

        // Publish:
        motor_left.status().current_a = read_current(LEFT);
        auto status = motor_left.publish_status();
        if (status != Status::OK) {
            // エラーハンドリング
        }
    }
