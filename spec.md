...
message MethodOptions {
  ...
  bool   read_only      = 3;  // Param: 読み取り専用か (オーバーライド用)
  string ros2_name      = 4;  // Topic/Param/Service の ROS 2 名オーバーライド
  string ros2_srv_type  = 5;  // Service: 既存 ROS 2 サービス型 (例: "std_srvs/srv/SetBool")
}
...
│  (unary, request が google.protobuf.Empty)  │ 読み取り専用        │ (read-only)   │
└───────────────────────────────────────────────────────────────────────────┘
```
...
- スキーマハッシュは、`.proto` の構造的内容と ROS 2 のマッピング情報から **コンパイル時に計算** された 32-bit (FNV-1a) ハッシュ値である。
...
※ OP (OPERATIONAL) 状態以降はネットワーク帯域節約のため、`num_nodes = 0` としてノード情報を省略したベース部分（7 bytes）のみを送信する。PREOP 状態までは、全ノードの情報を送信しきるまで複数の HEARTBEAT フレームを周期的に送信する（最大7ノード/フレーム）。
```
...
データペイロードにはネットワーク全体の時刻情報等を含めることが可能。
...
マスターはデータを経由させることなく、あるデバイスが出力したデータを別のデバイスの Rx 用トピックとして流し込むことができる（デバイス間直接ルーティング）。

- Device A に TX PDO 0x181 を設定
- Device B に RX PDO 0x181 を設定
デバイス B は自身の CAN FD パスでハードウェアレベルで 0x181 を受け取り、CPU 割り込みで直接コントローラ等に適用する。
...
デバイスの ROM (ファームウェア内) に配置されるディスクリプタは、`protoc` プラグインが生成した **不透明な定数バイナリ配列 (Blob)** である (`const uint8_t[]`)。
ファームウェア自身はディスクリプタをパースせず、ブリッジからの DISC 要求にこの Blob を返す機能のみ持つ。

ブリッジ側は受信した Blob のフィールド（`node_type_name`, `num_topics`, `ros2_msg_type`, `periodic` 等）をパースし、ROS 2 ノードを動的に組み立てる。
